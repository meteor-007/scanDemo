...
//        String[] testPass = readTxt("D:\\数据库漏洞扫描\\数据库漏洞扫描\\密码字典\\mysql.txt");
        String[] testPass = FileUtils.toArrayByFileReader1("D:\\dictionary.txt");

        String[] ar = new String[testPass.length];

        for (int i = 0; i < testPass.length; i++) {
//            list.add(TestPassword.mysql(testPass[i]));
...
//            ar[i] = WpUtils.sysbase(testPass[i]);
...
//            System.out.println(testPass[i]);
        ...
        fos = new FileOutputStream(new File("D:\\gbase.txt"));
...
//        fos = new FileOutputStream(new File("D:\\数据库漏洞扫描\\数据库漏洞扫描\\密码字典\\4位\\gbase.txt"));
